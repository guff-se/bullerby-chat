...
// Current touch state (updated by polling timer)
...
static void cst816d_read(void)
{
    ...
    uint8_t reg = 0x02;
    uint8_t data[6] = {0};
    esp_err_t ret = i2c_master_transmit_receive(s_dev, &reg, 1, data, 6, 10);
...
esp_err_t touch_init(lv_display_t *disp)
{
    ...
    // Verify communication: read chip ID from register 0xA3
    uint8_t reg = 0xA3;
    uint8_t chip_id = 0;
    esp_err_t ret = i2c_master_transmit_receive(s_dev, &reg, 1, &chip_id, 1, 100);
    if (ret == ESP_OK) {
