...
- **UI (`ui_app.c`):** **Home** = **ring of family circles** (even angular spacing, emoji inside each); **center message bubble** when the dummy inbox has items (tap → model mark-read / state); with **`CONFIG_BULLERBY_ENABLE_NET=y`**, if the relay is not ready (`net_intercom_ui_ready()` false), the ring and bubble are hidden and the home screen shows **Frånkopplad** instead of a dummy ring. **Recording** = full-bleed family colour, title bar, **Record/Stop**, back, **random Swedish send toast**, **30 s max** UI timer; **Record/Stop** drives **real** I2S capture on `audio_task` (same pipeline as **BOOT** hold: PCM → loopback + optional upload when `net_intercom_ui_ready()`). No horizontal strip / zoom overlay in current tree — superseded by v2 (see **ui-spec.md** implementation notes).
...
- [x] **Device identity:** `model_my_family_id` in NVS namespace `bullerby`, key `family_id`; first boot uses `CONFIG_BULLERBY_DEFAULT_FAMILY_ID` (repo default **1** / ANSUND in `sdkconfig.defaults`). **`model_set_my_family_id()`** for later provisioning. With **`CONFIG_BULLERBY_ENABLE_NET`** + **`CONFIG_BULLERBY_DEVICE_ID_FROM_MAC`**, `model_init` skips NVS `family_id` so a stale value (e.g. old default **8** / TADAA) is not shown before `GET …/config`; after a successful apply, NVS is updated. With net + MAC off, NVS + Kconfig behave as before. **`CONFIG_BULLERBY_DEVICE_ID_FROM_MAC`** (default **on** when networking is enabled) sets `X-Device-Id` to `esp-` + 12 hex WiFi MAC; add each `esp-…` under `devices[].id` in `server/config/bullerby.json`. Turn it off only for fixed-string lab ids.
...
- [x] **Identity:** `net/identity.c` reads NVS `bullerby` / `server_url` (and optional `device_id` only when MAC derivation is off); falls back to Kconfig `SERVER_URL` / `DEVICE_ID`. With default Kconfig, WiFi MAC replaces `device_id` with `esp-…` (stable across `idf.py flash`). Trailing slash on `server_url` stripped. Auth is just `X-Device-Id` — no shared secret.
